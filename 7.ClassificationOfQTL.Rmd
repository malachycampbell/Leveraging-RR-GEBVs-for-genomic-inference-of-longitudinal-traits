...
TP.p <- as.matrix( read.table("/Users/malachycampbell/Desktop/RRgwas_git/CompCluster/TP/pvals.txt", sep = "\t", header = T, row.names = 1) )
RR.p <- as.matrix( read.table("/Users/malachycampbell/Desktop/RRgwas_git/CompCluster/RR/pvals.txt", sep = "\t", header = T, row.names = 1) )
RR.p <- RR.p + 1E-10
TP.p <- TP.p + 1E-10

MAP <- read.table("/Users/malachycampbell/Desktop/RRgwas_git/CompCluster/MAP_maf0.05.txt", header = F, sep = "\t")
...
length(unique(RR.p$Var1)) #717
length(unique(TP.p$Var1)) #379

write.table(TP.p[c("V1", "V4", "V5", "Var2")],
            "/Users/malachycampbell/Desktop/RRgwas_git/TP_GP/bedtools/TP.p.BED",
            sep = "\t", col.names = F, row.names = F, quote = F)
write.table(RR.p[c("V1", "V4", "V5", "Var2")],
            "/Users/malachycampbell/Desktop/RRgwas_git/RR_GP/bedtools/RR.p.BED",
            sep = "\t", col.names = F, row.names = F, quote = F)
write.table(TP.p[c("V1", "V4", "V5", "Var1")],
            "/Users/malachycampbell/Desktop/RRgwas_git/TP_GP/bedtools/TP.pSNPs.BED",
            sep = "\t", col.names = F, row.names = F, quote = F)
write.table(RR.p[c("V1", "V4", "V5", "Var1")],
            "/Users/malachycampbell/Desktop/RRgwas_git/RR_GP/bedtools/RR.pSNPs.BED",
            sep = "\t", col.names = F, row.names = F, quote = F)
```

```{bash find QTLs, eval = F}
cd /Users/malachycampbell/Desktop/RRgwas_git/TP_GP/bedtools/
bedtools merge -i TP.p.BED -d 200000 -nms > TP.p.QTL.txt
bedtools merge -i TP.pSNPs.BED -d 200000 -nms > TP.p.QTLSNPs.txt
less TP.p.QTL.txt | wc -l #153

cd /Users/malachycampbell/Desktop/RRgwas_git/RR_GP/bedtools/
bedtools merge -i RR.p.BED -d 200000 -nms > RR.p.QTL.txt
bedtools merge -i RR.pSNPs.BED -d 200000 -nms > RR.p.QTLSNPs.txt
less RR.p.QTL.txt | wc -l #342
...
```{r split column}
RR.QTLs <- read.table("/Users/malachycampbell/Desktop/RRgwas_git/RR_GP/bedtools/RR.p.QTL.txt",
                      header = F, sep = "\t")
...
dim(RR.QTLs[RR.QTLs$NoDays == 20 ,]) #128 QTL detected on all days
dim(RR.QTLs[RR.QTLs$NoDays != 20 ,]) #214
dim(RR.QTLs[RR.QTLs$NoDays >= 10 ,]) #179
dim(RR.QTLs[RR.QTLs$NoDays == 1 ,]) #39
```
...
library(plyr)
RR.QTLs <- read.table("/Users/malachycampbell/Desktop/RRgwas_git/RR_GP/bedtools/RR.p.QTL.txt", header = F, sep = "\t")
...
RR.QTLs.perst <- RR.QTLs[RR.QTLs$NoDays == 20 ,] #QTLs detected on all days, 128
RR.QTLs.mid.long <- RR.QTLs[(RR.QTLs$NoDays > 12 & RR.QTLs$NoDays < 20) ,] #32
RR.QTLs.mid.sh <- RR.QTLs[(RR.QTLs$NoDays > 5 & RR.QTLs$NoDays < 13) ,] #55
RR.QTLs.short <- RR.QTLs[RR.QTLs$NoDays < 6 ,] #127
...
RR.mat.per <- RR.mat[order(RR.mat$min) ,][1:20,]
...
Cnt.day.ML <- apply(RR.mat[4:23], 2, function(x) sum(x < 1e-04)/32)
plot(1:20, Cnt.day.ML, type = "l", ylim = c(0, 1))

RR.mat.ML <- RR.mat[order(RR.mat$min) ,][1:20,]
...
Cnt.day.MS <- apply(RR.mat[4:23], 2, function(x) sum(x < 1e-04)/55)
plot(1:20, Cnt.day.MS, type = "l", ylim = c(0, 1))

RR.mat.MS <- RR.mat[order(RR.mat$min) ,][1:20,]
...
Cnt.day.S <- apply(RR.mat[4:23], 2, function(x) sum(x < 1e-04)/127)
plot(1:20, Cnt.day.MS, type = "l", ylim = c(0, 1))

RR.mat.S <- RR.mat[order(RR.mat$min) ,][1:20,]
```

## Plot frequencies for each class
```{r plot frequencies for each class}
#pdf("/Users/malachycampbell/Desktop/RRgwas_git/Figures/QTL.freq.pdf", h = 1.9, w=3.2, useDingbats = F, pointsize = 10)
...
P.mat.all <- rbind(P.mat.per[1:5,], P.mat.ML[1:5,], P.mat.MS[1:5,], P.mat.S[1:5,])
...
pheatmap(P.mat.all,
         ...
         fontsize_number = 3.5,
         breaks = myBreaks,
         filename = "/Users/malachycampbell/Desktop/RRgwas_git/Figures/Fig3.pdf",
         width = 7,
         height = 5,
         show_rownames = T,
         show_colnames = T,
         labels_row = paste0(RR.mat$V1, ": ", RR.mat$V2, " - ", RR.mat$V3),
         labels_col = seq(1:20)
)
```